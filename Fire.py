...
class FireBall:
    ...
    def update(self):
        ...
        if self.dir==0:
            self.x -= self.velocity+RUN_SPEED_PPS
        self.x = clamp(25, self.x, Server.ground.w // 2 - 550)

        for monsters in Server.monsters:
            if collide(self,monsters):
                game_world.remove_object(self)
                game_world.remove_object(monsters)

        for tmonster in Server.tutles:
            if collide(self,tmonster):
                game_world.remove_object(self)
                game_world.remove_object(tmonster)
